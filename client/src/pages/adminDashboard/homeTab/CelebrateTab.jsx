...
import React, { useState, useEffect } from 'react';
import { adminAPI } from '../../../config/api';
import { MdUpload, MdDelete, MdVisibility, MdVisibilityOff, MdAdd, MdClear, MdWork, MdLink, MdImage, MdRefresh, MdPerson, MdBusiness, MdVideoLibrary } from 'react-icons/md';
...
function CelebrateTab() {
  ...
  const fetchCelebrates = async () => {
    ...
    try {
      const res = await adminAPI.getCelebrates();
      // Ensure we set an array, even if res.data is not an array
      const fetchedData = Array.isArray(res.data) ? res.data : [];
      setCelebrates(fetchedData);
      console.log('Fetched celebrates:', fetchedData);
    } catch (err) {
      console.error('Error fetching celebrates:', err);
      setMessage('❌ Error fetching success stories');
      setCelebrates([]); // Reset to empty array on error
    } finally {
      setFetching(false);
    }
  };
  ...
  const handleChange = (e) => {
    ...
    if (files && files[0]) {
      ...
      // Create preview for both media and backgroundVideo
      const reader = new FileReader();
      reader.onload = (e) => {
        if (name === 'media') {
          setMediaPreview(e.target.result);
        } else if (name === 'backgroundVideo') {
          setBgVideoPreview(e.target.result);
        }
      };
      reader.readAsDataURL(file);
    } else {
      setForm((prev) => ({ ...prev, [name]: value }));
      // Clear preview if URL field is cleared manually or changed
      if (name === 'mediaUrl') {
        if (!value) setMediaPreview(null);
        else {
          // If it's a valid URL, we could show a preview, but for image URLs we can set preview directly
          if (form.mediaType === 'image' && (value.match(/\.(jpeg|jpg|gif|png)$/i) || value.startsWith('http'))) {
            setMediaPreview(value);
          } else if (form.mediaType === 'video' && (value.match(/\.(mp4|webm|ogg)$/i) || value.startsWith('http'))) {
            setMediaPreview(value);
          }
        }
      }
      if (name === 'backgroundVideoUrl') {
        if (!value) setBgVideoPreview(null);
      }
    }
  };
  ...
  const clearMedia = (type) => {
    if (type === 'media') {
      setForm(prev => ({ ...prev, media: null, mediaUrl: '' }));
      setMediaPreview(null);
    } else if (type === 'backgroundVideo') {
      setForm(prev => ({ ...prev, backgroundVideo: null, backgroundVideoUrl: '' }));
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.media && !form.mediaUrl) {
      setMessage('❌ Media file or URL is required.');
      return;
    }

    if (form.mediaType === 'video' && !form.backgroundVideo && !form.backgroundVideoUrl) {
      setMessage('❌ Background video is required for video stories.');
      return;
    }
    ...
    try {
      ...
      } else if (form.backgroundVideoUrl) {
        formData.append('backgroundVideoUrl', form.backgroundVideoUrl);
      }
  ...
  const toggleStatus = async (id) => {
    try {
      ...
      setMessage('❌ Failed to update status');
      fetchCelebrates(); // Revert optimistic update
    }
  };
  ...
  const renderMediaPreview = () => {
    ...
    // For URL preview or file preview
...
const renderStoryMedia = (celebrate) => {
  return (
    <div className="space-y-4">
      ...
      <div>
        ...
        {celebrate.mediaType === "video" ? (
          <video
            src={celebrate.mediaUrl}
            controls
      ...
      {celebrate.backgroundVideoUrl && (
        <div>
          ...
          <video
            src={celebrate.backgroundVideoUrl}
            controls
            autoPlay
            muted
            loop
            ...
            <div className="p-6">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="space-y-6">
                  <div>
                    <label className="flex items-center gap-2 text-sm font-medium text-gray-700 mb-2">
                      <MdWork className="text-gray-400" /> Story Title *
                    </label>
                    <input 
                      type="text"
                      name="title"
                      value={form.title}
                      onChange={handleChange}
                      placeholder="e.g., John Doe's Journey to Google"
                      className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                      required
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex gap-4">
                      <label className="flex items-center gap-2">
                        ...
                        />
                        <MdImage className="text-gray-500" /> Image
                      </label>
                      <label className="flex items-center gap-2">
                        <input
                          type="radio"
                          ...
                          className="w-4 h-4 text-blue-600"
                        />
                        <MdVideoLibrary className="text-gray-500" /> Video
                      </label>
                    </div>
                  </div>
                  ...
                  <div>
                    <label className="flex items-center gap-2 text-sm font-medium text-gray-700 mb-2">
                      {form.mediaType === 'video' ? <MdVideoLibrary className="text-gray-400" /> : <MdImage className="text-gray-400" />}
                      {form.mediaType === 'video' ? 'Video File/URL *' : 'Image File/URL *'}
                    </label>
                        ...
                        {mediaPreview ? (
                          ...
                          <div className="space-y-2">
                            {form.mediaType === 'video' ? (
                              <MdVideoLibrary className="text-gray-400 text-2xl mx-auto" />
                            ) : (
                              <MdImage className="text-gray-400 text-2xl mx-auto" />
                            )}
                            <label className="cursor-pointer bg-blue-600 text-white px-3 py-2 rounded-lg hover:bg-blue-700 inline-block text-sm">
                            ...
                            <input
                              ...
                              value={form.mediaUrl}
                              onChange={handleChange}
                              placeholder={`Enter ${form.mediaType === 'video' ? 'video' : 'image'} URL`}
                              className="w-full p-2 border border-gray-300 rounded-lg text-sm"
                            />
                            <p className="text-xs text-gray-500">
                              {form.mediaType === 'video' ? 'MP4, WebM up to 50MB' : 'PNG, JPG, JPEG up to 5MB'}
                            </p>
                          </div>
                        )}
                      </div>
                  ...
                  {form.mediaType === 'video' && (
                    <div>
                      <label className="flex items-center gap-2 text-sm font-medium text-gray-700 mb-2">
                        <MdVideoLibrary className="text-gray-400" /> Background Video (Optional)
                      </label>
                      <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center">
                        {bgVideoPreview ? (
                          <div className="relative">
                            ...
                            <button
                              type="button"
                              onClick={() => clearMedia('backgroundVideo')}
                              className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full p-1 hover:bg-red-600"
                            >
                              <MdClear className="w-3 h-3" />
                            </button>
                  ...
                  {mediaPreview ? (
                    <div className="bg-white border border-gray-200 rounded-xl p-6">
                      <div className="space-y-4">
                        ...
                        <div>
                          <h4 className="font-semibold text-gray-900 text-lg">{form.title || 'Story Title'}</h4>
                          <p className="text-gray-500 text-sm mt-1">
                            Type: {form.mediaType === 'video' ? 'Video Story' : 'Image Story'}
                          </p>
                          {form.mediaType === 'video' && bgVideoPreview && (
                            <p className="text-gray-500 text-xs mt-1">✓ Background video attached</p>
                          )}
                        </div>
                      </div>
                    </div>
          ...
          {activeTab === 'manage' && (
            <div className="p-6">
              ...
              {fetching ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {celebrates.map(celebrate => (
                    <div key={celebrate._id} className="bg-white border border-gray-200 rounded-xl overflow-hidden hover:shadow-md transition-shadow">
                      <div className="relative">
                        {renderStoryMedia(celebrate)}
                        {celebrate.backgroundVideoUrl && (
                      ...
                      <div className="p-5">
                        <h4 className="font-semibold text-gray-900 text-lg mb-2">{celebrate.title}</h4>
                        <div className="flex items-center gap-2 mb-3">
                          <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${celebrate.status === 'active' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>
                            {celebrate.status === 'active' ? 'Active' : 'Inactive'}
                          </span>
                          <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                            {celebrate.mediaType === 'video' ? 'Video' : 'Image'}
                          </span>
                        </div>
                        <div className="flex gap-2 mt-4 pt-4 border-t border-gray-100">
                          <button 
