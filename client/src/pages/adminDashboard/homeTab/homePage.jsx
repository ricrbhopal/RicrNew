...
const HomePage = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-3 sm:p-4 md:p-6">
      ...
      {/* 🔽 UNIVERSAL DROPDOWN (Desktop + Mobile) */}
      <div className="mb-6">
        <select
          value={active}
          onChange={(e) => setActive(e.target.value)}
          className="w-full md:w-[400px] px-4 py-3 text-sm md:text-base bg-white border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-700"
        >
          {tabs.map((tab) => (
            <option key={tab.id} value={tab.id}>
              {tab.label}
